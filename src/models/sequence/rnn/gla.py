...
from src.models.sequence.rnn.gla_triton.intra_chunk_contribution.fn import intra_chunk_onc
from src.models.sequence.rnn.gla_triton.inter_chunk_contribution.fn import inter_chunk_onc
...
class GatedLinearAttention(nn.Module):
    ...
    def forward(self, x, return_attention=False, input_ids=None, attn_chunk_size=16):
        assert return_attention is False, "not implemented"

        # padding the length to be multiple of attn_chunk_size
        ...
        if seq_len % attn_chunk_size != 0:
            ...
            pad_mat = torch.randn(bz, padding_length, hidden_dim).to(x.device)
            x = torch.cat([x, pad_mat], dim=1)
        else:
            padding_length = None
        ...
        if self.use_gv:
            ...
            v_gate = None
        g = self.g_proj(x)
        output = self.gated_linear_attention(q, k, v, k_gate, v_gate, num_head=self.num_heads, chunk_size=attn_chunk_size)
