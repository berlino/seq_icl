...
class SequenceLightningModule(pl.LightningModule):
    ...
    def _writes_to_file(self, prefix, x, y, batch, dfas, ngram=3, hidden_outputs=None, char_labels=None, save_probs=False):
        inputs = batch[0].detach().cpu().numpy()
        ...
        if hidden_outputs is not None:
            ...
            if isinstance(hidden_outputs, tuple):
                ...
                if attention_scores is not None:
                    attention_contexts = [attention_score[0] for attention_score in attention_scores]
                    attention_scores = [attention_score[1] for attention_score in attention_scores]
                    # attention_context, attention_scores = attention_scores

            for i in range(200):

                path = f"generations/{self.current_epoch}_{prefix}_batch/{i}.pkl.gz"
    ...
    def _shared_step(self, batch, batch_idx, prefix="train"):
        ...
        if ("final" in  metric_prefix) and (prefix == "test" or prefix == "val"):
            return_hidden_outputs = self.hparams.dataset.num_examples == 2500 or self.hparams.dataset.num_examples == 40000
        else:
    ...
    def _eval_dataloaders(self):
        ...
        val_loaders = self.dataset.train_dataloader(**self.hparams.loader)
...
def main(config: OmegaConf):
    ...
    run_dir = os.getcwd()
    print("removing run_dir: ", run_dir)
    # check if there is a folder named generations
    if os.path.isdir("generations"):
        shutil.rmtree("generations")
    # check if there is a folder named samples
    if os.path.isdir("samples"):
        shutil.rmtree("samples")
    # check if there is a folder named wandb
